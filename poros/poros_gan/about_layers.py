...
def generative_net(x: tf.Tensor, units: list, kernel_initializer=tf.nn.leaky_relu):
    ...
    with tf.variable_scope("generative_net"):
        inputs = x
        for i, nums in enumerate(units[:-1]):
            hidden_output = tf.layers.dense(inputs=inputs, units=units[i], kernel_initializer=kernel_initializer)
            inputs = hidden_output
        output = tf.layers.dense(inputs=inputs, units=units[-1], kernel_initializer=tf.nn.sigmoid)
    return output


def discriminative_net(x: tf.Tensor, units: list, kernel_initializer=tf.nn.leaky_relu):
    """
